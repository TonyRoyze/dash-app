...
export default function AdidasSalesDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (parsedData.length === 0) {
        ...
        setData(sampleData);
        setDataDateRange(getDateRange(sampleData));
      } else {
        setData(parsedData);
        setDataDateRange(getDateRange(parsedData));
      }
    } catch (err) {
      ...
      setDataDateRange(getDateRange(sampleData));
      setError(
        `${err instanceof Error ? err.message : "Failed to load data"} (using sample data as fallback)`,
      );
    } finally {
      setLoading(false);
    }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen p-6">
        <Alert className="max-w-md">
          ...
          <AlertDescription className="mt-2">
            <p className="mb-4">
              No sales data could be loaded. Please check the data source.
            </p>
            <Button onClick={loadData}>Retry</Button>
          </AlertDescription>
        </Alert>
  ...
  const filteredData = data.filter((record) => {
    const regionMatch =
      selectedRegions.length === 0 || selectedRegions.includes(record.Region);
    const productMatch =
      selectedProducts.length === 0 ||
      selectedProducts.includes(record.Product);
    ...
    return regionMatch && productMatch && dateMatch;
  });

  const totalSales = filteredData.reduce(
    (sum, record) => sum + record["Total Sales"],
    0,
  );
  const totalProfit = filteredData.reduce(
    (sum, record) => sum + record["Operating Profit"],
    0,
  );
  const totalUnits = filteredData.reduce(
    (sum, record) => sum + record["Units Sold"],
    0,
  );
  const avgMargin = totalProfit / totalSales || 0;
  const uniqueRetailers = new Set(filteredData.map((record) => record.Retailer))
    .size;

  const regionData = aggregateByRegion(filteredData);
  const productData = aggregateByProduct(filteredData).slice(0, 10);
  const monthlyTrends = getMonthlyTrends(filteredData);

  const salesMethodData = filteredData.reduce(
    (acc, record) => {
      acc[record["Sales Method"]] =
        (acc[record["Sales Method"]] || 0) + record["Total Sales"];
      return acc;
    },
    {} as Record<string, number>,
  );

  const salesMethodChart = Object.entries(salesMethodData).map(
    ([method, sales]) => ({
      method,
      sales,
    }),
  ...
  const handleQuickDateChangeAction = (range: {
    from: Date | undefined;
    to: Date | undefined;
  }) => {
    setDateRange(range);
        ...
        <div className="bg-card rounded-lg shadow-sm border border-border p-6 mb-8">
          ...
          {dataDateRange && (
            <div className="mt-6 pt-4 border-t border-gray-100">
              <QuickDateFilters
                onDateRangeChangeAction={handleQuickDateChangeAction}
              ...
              <div className="flex flex-wrap gap-2">
                {selectedRegions.map((region) => (
                  <Badge
                    key={`region-${region}`}
                    variant="outline"
                    className="flex items-center gap-1 p-3 bg-background"
                  >
                    Region: {region}
                    <X
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <Card className="hover:shadow-md transition-shadow">
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">
                  {(avgMargin * 100).toFixed(1)}% margin
                </p>
              </CardContent>
            </Card>
            ...
            <Card className="hover:shadow-md transition-shadow">
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">
                  Avg:{" "}
                  {filteredData.length
                    ? Math.round(totalUnits / filteredData.length)
                    : 0}{" "}
                  per transaction
                </p>
              </CardContent>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card className="hover:shadow-md transition-shadow">
                  <CardHeader>
                    ...
                    <CardDescription>
                      Best performing retail partners
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <CustomBarChart
